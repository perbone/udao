...
public enum IsolationLevel
{
     ...
     * This level includes the prohibitions in {@code REPEATABLE_READ} and further prohibits the
     * situation where one transaction reads all rows that satisfy a {@code WHERE} condition, a
    ...
    public int level()
    {
        if (this == NONE)
            return 0;
        else if (this == READ_UNCOMMITTED)
            return 1;
        else if (this == READ_COMMITTED)
            return 2;
        else if (this == REPEATABLE_READ)
            return 4;
